        ...
        if (franchise) {
            if (request_data_body.server_token !== null && franchise.server_token !== request_data_body.server_token) {
                ...
                Order.findOne({ _id: request_data_body.order_id }).then(order_detail => {
                    ...
                    Provider.findOne({ _id: order_detail.current_provider }).then(provider_detail => {
            ...
            if (request_data_body.server_token !== null && franchise_detail.server_token !== request_data_body.server_token) {
                ...
                for (var i = 0; i < franchise_detail.store_ids.length; i++) {
                    ...
                    if (i + 1 == franchise_detail.store_ids.length) {
                        ...
                        Setting.findOne({}, { "password": 0, "email": 0 }).then(setting_detail => {
                            var user_query = {
                                $lookup:
                                {
                                    from: "users",
                            ...
                            Order.aggregate([condition, condition1, condition2, user_query, store_query, array_to_json_store_query, array_to_json1]).then(orders => {
            ...
            if (request_data_body.server_token !== null && franchise_detail.server_token !== request_data_body.server_token) {
                ...
                for (var i = 0; i < franchise_detail.store_ids.length; i++) {
                    ...
                    if (i + 1 == franchise_detail.store_ids.length) {
                        ...
                        Order.aggregate([condition, condition1, condition2, user_query, provider_query, order_payment_query, store_query, array_to_json_store_query, array_to_json1, array_to_json2, sort]).then(orders => {
            ...
            if (request_data_body.server_token !== null && franchise_detail.server_token !== request_data_body.server_token) {
                ...
                for (var i = 0; i < franchise_detail.store_ids.length; i++) {
                    ...
                    if (i + 1 == franchise_detail.store_ids.length) {
                        ...
                        Order.aggregate([condition, condition1, condition2, user_query, provider_query, order_payment_query, array_to_json1, array_to_json_order_payment_query, store_query, array_to_json_store_query, payment_gateway_query, search, count]).then(orders => {
                            if (!orders || orders.length === 0) {
                                ...
                                Order.aggregate([condition, condition1, condition2, user_query, provider_query, order_payment_query, array_to_json1, array_to_json_order_payment_query, store_query, array_to_json_store_query, payment_gateway_query, sort, search, skip, limit]).then(orders => {
                                    if (error) {
                                        response_data.json({ success: false, error_code: ORDER_ERROR_CODE.ORDER_NOT_FOUND });
                                    } else {
                                        response_data.json({
            ...
            if (request_data_body.server_token !== null && franchise_detail.server_token !== request_data_body.server_token) {
                ...
                for (var i = 0; i < franchise_detail.store_ids.length; i++) {
                    ...
                    if (i + 1 == franchise_detail.store_ids.length) {
                        ...
                        Order.aggregate([condition, condition1, user_query, order_payment_query, provider_query, review_query, array_to_json_user_detail, array_to_json_order_payment_query, store_query, array_to_json_store_query, payment_gateway_query, search, filter, count]).then(orders => {
                            if (!orders || orders.length === 0) {
                                response_data.json({ success: false, error_code: ORDER_ERROR_CODE.ORDER_NOT_FOUND, pages: 0 });
                            } else {
                                var pages = Math.ceil(orders[0].total / number_of_rec);
                                Order.aggregate([condition, condition1, user_query, order_payment_query, provider_query, review_query, array_to_json_user_detail, array_to_json_order_payment_query, store_query, array_to_json_store_query, payment_gateway_query, sort, search, filter, skip, limit]).then(orders => {
                                    if (error) {
                                        response_data.json({ success: false, error_code: ORDER_ERROR_CODE.ORDER_NOT_FOUND });
                                    } else {
                                        response_data.json({
                ...
                for (var i = 0; i < franchise_detail.store_ids.length; i++) {
                    ...
                    if (i + 1 == franchise_detail.store_ids.length) {
                        Setting.findOne({}).then(setting_detail => {
                            ...
                            Order.aggregate([condition, condition1, user_query, array_to_json1, order_payment_query, array_to_json_order_payment_query, store_query, array_to_json_store_query, payment_gateway_query, search, count]).then(orders => {
                                if (!orders || orders.length === 0) {
                                    response_data.json({ success: false, error_code: ORDER_ERROR_CODE.ORDER_NOT_FOUND, pages: 0 });
                                } else {
                                    var pages = Math.ceil(orders[0].total / number_of_rec);
                                    Order.aggregate([condition, condition1, user_query, array_to_json1, order_payment_query, array_to_json_order_payment_query, store_query, array_to_json_store_query, payment_gateway_query, sort, search, skip, limit]).then(orders => {
                                        if (error) {
                                            response_data.json({ success: false, error_code: ORDER_ERROR_CODE.ORDER_NOT_FOUND });
                                        } else {

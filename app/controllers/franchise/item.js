...
exports.upload_item_image = function (request_data, response_data) {
    var request_data_body = request_data.body;
    FranchiseItem.findOne({ _id: request_data_body.item_id }).then(item => {
        if (item) {
...
exports.get_for_store_product_item_list = function (request_data, response_data) {
    ...
    Franchise.findOne({ _id: franchise_id }).then(detail => {
        if (detail) {
            if (request_data_body.server_token !== null && detail.server_token !== request_data_body.server_token) {
                response_data.json({ success: false, error_code: ERROR_CODE.INVALID_SERVER_TOKEN });
            } else {
                var ads = [];
                Store.findOne({ _id: request_data_body.store_id }).then(detail => {
...
exports.get_store_product_item_list = function (request_data, response_data) {
    ...
    Franchise.findOne({ _id: id }).then(detail => {
        if (detail) {
            if (request_data_body.server_token !== null && detail.server_token !== request_data_body.server_token) {
                response_data.json({ success: false, error_code: ERROR_CODE.INVALID_SERVER_TOKEN });
            } else {
...
exports.get_item_list = function (request_data, response_data) {
    var request_data_body = request_data.body;
    var franchise_id = request_data_body.franchise_id;
    Franchise.findOne({ _id: franchise_id }).then(franchise_detail => {
        if (franchise_detail) {
            if (request_data_body.server_token !== null && franchise_detail.server_token !== request_data_body.server_token) {
                response_data.json({ success: false, error_code: ERROR_CODE.INVALID_SERVER_TOKEN });
            } else {
                Country.findOne({ _id: franchise_detail.country_id }).then(country_data => {
                    if (country_data) {
                        var currency = country_data.currency_sign;
                        var products_array = {
                            $lookup:
    ...
    Franchise.findOne({ _id: franchise_id }).then(franchise_detail => {
        if (franchise_detail) {
            if (request_data_body.server_token !== null && franchise_detail.server_token !== request_data_body.server_token) {
                ...
                FranchiseItem.findOne({ _id: item_id }).then(item_data => {
                    if (item_data) {
...
exports.is_item_in_stock = function (request_data, response_data) {
    var request_data_body = request_data.body;
    var item_id = request_data_body.item_id;
    FranchiseItem.findOne({ _id: item_id }).then(item => {
        if (item) {
    ...
    Franchise.findOne({ _id: request_data_body.franchise_id }).then(franchise_detail => {
        if (franchise_detail) {
            if (request_data_body.server_token !== null && franchise_detail.server_token !== request_data_body.server_token) {
                ...
                name = name.charAt(0).toUpperCase() + name.slice(1);
                request_data_body.name = name;
                FranchiseItem.findOneAndUpdate({ _id: item_id }, request_data_body, { new: true }).then(item_data => {
    ...
    Franchise.findOne({ _id: request_data_body.franchise_id }).then(franchise_detail => {
        if (franchise_detail) {
            if (request_data_body.server_token !== null && franchise_detail.server_token !== request_data_body.server_token) {
                ...
                FranchiseItem.findOne({ _id: request_data_body._id }).then(item => {
                    if (item) {

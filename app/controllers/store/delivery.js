                ...
                if (store_detail) {
                    ...
                    } else
                    {
                        ...
                        var array_to_json_cart_query = {$unwind: "$cart_detail"};
                         ...
                         var order_condition = {$match: {
                                $and: [
                                    {'order_detail.order_status': {$ne: ORDER_STATE.STORE_REJECTED}},
                                    {'order_detail.order_status': {$ne: ORDER_STATE.STORE_CANCELLED}},
                                    {'order_detail.order_status': {$ne: ORDER_STATE.CANCELED_BY_USER}},
                                    {'order_detail.order_status': {$ne: ORDER_STATE.ORDER_COMPLETED}},
                                ]
                            }
                        }
                        ...
                        Request.aggregate([delivery_status_condition, delivery_status_manage_id_condition, user_query, provider_query, orders_array_unwind, order_query, order_payment_query, array_to_json_user_query, array_to_json_order_query, store_id_condition,cart_query, order_condition, array_to_json_order_payment_query,array_to_json_cart_query, sort]).then((requests) => {
                            ...
                            var condition1 = {$match: {'is_business': {$eq: true}}};
                                ...
                                Service.aggregate([condition, condition1, type_condition, delivery_type_query, lookup, unwind, vehicle_condition, group]).then((services) => {
                                    if (services.length > 0) {
                                        ...
                                        response_data.json({success: true,
                                            ...
                                            requests: requests, admin_vehicles: services[0].admin_vehicles, vehicles: services[0].vehicles
                                        });
                                    } else {
                                        response_data.json({success: true,
                                            message: ORDER_MESSAGE_CODE.ORDER_LIST_FOR_DELIVERY_SUCCESSFULLY,
                                            requests: requests, admin_vehicles: [], vehicles: []
                                        });
                    ...
                    } else
                    {
                        ...
                        Request.aggregate([order_query,  array_to_json_order_query, store_id_condition, condition1, user_query, array_to_json1]).then((no_deliveryman_orders) => {
                            ...
                            Order.aggregate([condition, condition1, cart_query, array_to_json2, user_query, array_to_json1]).then((new_order_list) => {
                                ...
                                var condition = {$match: {'city_id': {$eq: store_detail.city_id}}};
                                var condition1 = {$match: {'is_business': {$eq: true}}};
                                Service.aggregate([condition, condition1, delivery_type_query, lookup, unwind, vehicle_condition, group]).then((services) => {
                                    if(services.length>0){
                                        services[0].admin_vehicles = services[0].admin_vehicles.filter(v => v != null);
                                        services[0].vehicles = services[0].vehicles.filter(v => v != null);
                                        response_data.json({
                                            ...
                                            vehicles: services[0].vehicles, admin_vehicles: services[0].admin_vehicles
                                        });
                                    } else {
                                        response_data.json({
                                            success: true,
            ...
            Store.findOne({_id: request_data_body.store_id}).then((store_detail) => {
                if (store_detail) {
                    ...
                    } else
                    {
                            ...
                            Order.aggregate([condition, condition1, condition2, user_query, array_to_json1, cart_query, array_to_json2]).then((no_deliveryman_orders) => {

                                var condition1 = {"$match": {'order_status': {$eq: ORDER_STATE.WAITING_FOR_ACCEPT_STORE}}};
                                Order.aggregate([condition, condition1, condition2, user_query, array_to_json1, cart_query, array_to_json2]).then((orders) => {
                                    if (orders.length == 0) {
                                        ...
                                        if (no_deliveryman_orders.length == 0) {
                                            ...
                                            Order.findOneAndUpdate({_id: no_deliveryman_orders[0]._id}, {store_notify: 1}).then((order) => {
                                                response_data.json({
                                                    success: false,
                                                    store_detail: store_detail,
                                                    error_code: ORDER_ERROR_CODE.ORDER_NOT_FOUND,
                    ...
                    } else
                    {
                        ...
                        Request.aggregate([delivery_status_condition,  request_status_condition, delivery_status_manage_id_condition, user_query, provider_query, provider_unwind, orders_array_unwind, order_query, order_payment_query, array_to_json_user_query, array_to_json_order_query, store_id_condition, order_condition, array_to_json_order_payment_query, payment_gateway_query, cart_query, array_to_json_cart_query, search, count]).then((requests) => {
                            ...
                            } else
                            {
                                ...
                                Request.aggregate([delivery_status_condition, request_status_condition, delivery_status_manage_id_condition, user_query, provider_query, provider_unwind, orders_array_unwind, order_query, order_payment_query, array_to_json_user_query, array_to_json_order_query, store_id_condition, order_condition, array_to_json_order_payment_query, payment_gateway_query, cart_query, array_to_json_cart_query, sort, search, skip, limit]).then((requests) => {
                                    if (requests.length === 0) {
                                        response_data.json({success: false, error_code: ORDER_ERROR_CODE.ORDER_NOT_FOUND});
                                    } else
                                    {
...
exports.history = function (request_data, response_data) {
    utils.check_request_params(request_data.body, [{name: 'start_date'}, {name: 'end_date'}, {name: 'page'}], function (response) {
        if (response.success) {
                ...
                if (store_detail) {
                    ...
                    } else
                    {
                        Country.findOne({_id: store_detail.country_id}).then((country_detail) => {
                            ...
                            Order.aggregate([condition, condition1, order_type_condition, created_by_condition, order_status_id_condition, user_query, order_payment_query, cart_query, array_to_json_cart_query, request_query, review_query, array_to_json_user_detail, array_to_json_order_payment_query, array_to_json_request_query, provider_query, array_to_json_provider_query, payment_gateway_query, pickup_type_condition, payment_status_condition, search, filter, count]).then((orders) => {
                                if (orders.length === 0) {
                                    response_data.json({
                                        success: false,
                                        error_code: ORDER_ERROR_CODE.ORDER_NOT_FOUND,
                                    ...
                                    Order.aggregate([condition, condition1,order_type_condition, created_by_condition, order_status_id_condition, user_query, order_payment_query, cart_query, array_to_json_cart_query, request_query, review_query, array_to_json_user_detail, array_to_json_order_payment_query, array_to_json_request_query, provider_query, array_to_json_provider_query, payment_gateway_query, pickup_type_condition, payment_status_condition, search, filter, sort, skip, limit]).then((orders) => {
                                        if (orders.length === 0) {
                                            response_data.json({success: false, error_code: ORDER_ERROR_CODE.ORDER_NOT_FOUND});
                                        } else {
                                            response_data.json({
                ...
                if (store_detail) {
                    ...
                    } else
                    {
                        Country.findOne({_id: store_detail.country_id}).then((country_detail) => {
                            ...
                            if(payment_mode != ''){
                                if(payment_mode == 1){
                                    payment_mode_condition = {$match: {'order_payment_detail.is_payment_mode_cash': {$eq: true}}}
                                } else {
                                    payment_mode_condition = {$match: {'order_payment_detail.is_payment_mode_cash': {$eq: false}}}
                                }
                            }
                            ...
                            var order_type_condition = {$match: {}}
                            if(order_type != ''){
                                order_type_condition = {$match: {'order_type': {$eq: Number(order_type)}}}
                            }
                            ...
                            //     {
                            //         var pages = Math.ceil(orders[0].total / number_of_rec);
                                    Order.aggregate([store_condition, order_status_condition, user_query, cart_query, order_payment_query, array_to_json_user_query, array_to_json_cart_query, array_to_json_order_payment_query, payment_gateway_query, payment_mode_condition, order_type_condition, pickup_type_condition, sort, search]).then((orders) => {
                                        if (orders.length === 0) {
                                            response_data.json({success: false, error_code: ORDER_ERROR_CODE.ORDER_NOT_FOUND});
                                        } else
                                        {
                                            response_data.json({success: true,
                                                ...
                                                currency_sign: country_detail.currency_sign, orders: orders});
                                        }
            ...
            Store.findOne({_id: request_data_body.store_id}).then((store_detail) => {
                if (store_detail) {
                    ...
                    } else
                    {
                        ...
                        Order.aggregate([order_condition, store_condition, user_query, order_payment_query, store_query, provider_query, array_to_json_user_detail, array_to_json_store_detail, array_to_json_order_payment_query
        ...
        if (response.success) {
            ...
            Store.findOne({_id: request_data_body.store_id}).then((store) => {
                if (store)
                {
                    ...
                    {
                        Request.findOne({_id: request_data_body.request_id}).then((request_detail) => {

                            Provider.findOne({_id: request_detail.current_provider}).then((provider_detail) => {

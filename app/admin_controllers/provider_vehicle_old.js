...
exports.provider_vehicle_update = function (request_data, response_data) {
    ...
    Provider_vehicle.findOneAndUpdate({ _id: request_data_body.provider_vehicle_id }, request_data_body, { new: true }).then(provider_vehicle => {
        ...
        if (detail) {
            ...
            Document_uploaded_list.aggregate([user_condition, user_type_id_condition, document_type_condition, document_query, array_to_json_document_query]).then(documents => {
                if (error || documents.length == 0) {
                    response_data.json({
                        success: false, error_code: DOCUMENT_ERROR_CODE.DOCUMENT_NOT_FOUND
                    });
                } else {
                    response_data.json({
                        success: true,
